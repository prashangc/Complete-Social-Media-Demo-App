...
class PostView(APIView):
    ...
    def get(self,request):
        ...
        for post in serializer.data:
            post_like = Like.objects.filter(post=post['id']).filter(like=True).count() # to count the total number of likes
            mylike = Like.objects.filter(post=post['id']).filter(user=request.user).first()
            if mylike:
                post['like'] = mylike.like
            else:
                post['like'] = False
            post['totallike']=post_like # to display the total number of likes
...
class AddALike(APIView):
    ...
    def post(self,request):
        try:
            ...
            # print(post_obj)
            like_obj = Like.objects.filter(post=post_obj).filter(user=c_user).first()
            if like_obj:
